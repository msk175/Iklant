...
function retrieveDetails(selectThis){
		...
		ajaxVariable = $.ajax({
			type: 'POST',
			url: 'http://'+ajaxcallip+'/mfi/api/1.0/client/ci/group/loansanction/'+mifosCustomerId+'/'+loanProductId+'/'+iklantGroupId+'/retrieveGroupInformation',
			success: function(data) {
				...
				for(var i=0;i<data.SanctionLoanHolderLocal.clientSelectForGroup.length;i++){
					...
					if(i == 0){
						...
						newContent+='</th>';
						newContent+='<th>';
						newContent+= "Active Loans";
						newContent+='</th>';
						newContent+='<th>';
						newContent+= "Overdue";
						newContent+='</th>';
						newContent+='<th>';
						newContent+= "Outstanding";
						newContent+='</th>';
						newContent+='<th>';	
						newContent+='Amount';						
						newContent+='</th>';					
						...
						newContent+='</td>';
						newContent+="<td id='accountOwnerId"+i+"'>";
						newContent+= data.SanctionLoanHolderLocal.clientSelectForGroup[i] + "-" + data.SanctionLoanHolderLocal.clientNameForGroup[i];
						newContent+='</td>';
						newContent+="<td id='activeLoans"+i+"'>";
						newContent+= data.SanctionLoanHolderLocal.noOfActiveLoans[i];
						newContent+='</td>';
						newContent+="<td id='overDue"+i+"'>";
						newContent+= data.SanctionLoanHolderLocal.outstandingAmount[i];
						newContent+='</td>';
						newContent+="<td id='outstanding"+i+"'>";
						newContent+= data.SanctionLoanHolderLocal.overdueAmount[i];
						newContent+='</td>';
						newContent+='<td>'; 
...
function editedClientAmount(i){
	...
	if(i==0){
		var firstClientAmt = parseInt($("#tabletext0").val()); 
...
for(var i=0;i<hiddenClientArrayLength;i++){
    ...
    // Added by chitra [Document shouldn't be generated for rejected clients]
        if(activeClientIdValue != ""){
            if(sa == true){
	...
	$.ajax({
		...
		complete: function() { 
			...
			url: 'http://'+ajaxcallip+'/mfi/api/1.0/client/ci/loanOffering/',
...
function generatePDF() {
	...
	if(formType != 0){
		...
		$.ajax({
				...
				async :false,
				contentType: 'application/json',
				url: 'http://'+ajaxcallip+'/mfi/api/1.0/client/ci/groups/member/loansanction/downloadPDF',
				success: function(success) {
...
function goToRepaymentSchedule(){
	...
	document.getElementById("LoanAccountFormID").action="/mfi/api/1.0/client/ci/loanrecovery/groups/"+globalCustomerNum+"/loanaccounts";
	document.getElementById("LoanAccountFormID").submit();
}
function showPageExpired(){
	document.getElementById("LoanAccountFormID").method='POST';
	document.getElementById("LoanAccountFormID").action="/mfi/api/1.0/client/ci/showErrorPage";
	document.getElementById("LoanAccountFormID").submit();
}
function back1(){
	var globalCustomerNum;
