...
function expensesCashPayment(){
	...
	if($("#fromOfficeHierarchyId").val() == ""){
        ...
        document.getElementById("accountsMenuFormID").action=localStorage.contextPath+"/accounts/submitCashPayment";
	...
	ajaxVariable = $.ajax({
		...
		url: 'http://'+ajaxcallip+'/mfi/api/1.0/accounts/submitCashPayment',
		success: function(data) {
			if(data.cashPayment == "success"){
				showCashPaymentForm();
			}else if(data.cashPayment == "failure"){
