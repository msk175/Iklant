...
function expensesBankReceipt(){
	...
	if($("#fromOfficeHierarchyId").val() == ""){
		...
		$(window).scrollTop(0);
	}else if(($("#chequeNoId").val() != "") && ($("#chequeNoId").val().length < 6 | $("#chequeNoId").val().length > 8)){
        ...
        document.getElementById("accountsMenuFormID").action=localStorage.contextPath+"/accounts/submitBankReceipt";
		...
		data.chequeNo 			 =  $("#chequeNoId").val();
		data.chequeDate 		 =  $("#chequeDateId").val();
	...
	ajaxVariable = $.ajax({
		...
		url: 'http://'+ajaxcallip+'/mfi/api/1.0/accounts/submitBankReceipt',
		success: function(data) {
			if(data.bankReceipt == "success"){
				showBankReceiptExpensesForm();
			}else if(data.bankReceipt == "failure"){
