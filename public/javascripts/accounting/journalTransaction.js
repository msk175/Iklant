...
function toOfficeIdChange(){
    ...
    for(var i=0;i<glCodeIdArray.length;i++){
...
function compareCredeitAndDebitAccountsOfficeId(fromAccountId,toAccountId){
	var officeIdArray = $("#fromGlCodeOffice").val().split(',');
	var glCodeIdArray = $("#fromGlcodeId").val().split(',');
	
	var toOfficeIdArray = $("#glCodeOffice").val().split(',');
	var toGlCodeIdArray = $("#glcodeId").val().split(',');
	
	var fromOfficeIndex = toGlCodeIdArray.indexOf(fromAccountId);
	var toOfficeIndex = toGlCodeIdArray.indexOf(toAccountId);
	
	if(fromAccountId == toAccountId){
		return (officeIdArray[fromOfficeIndex] == toOfficeIdArray[toOfficeIndex]);
	}else{
		return false;
	}
}
function submitJournalTransaction(){
	var allowedDecimals = parseInt($("#allowedDecimalsId").val(),10);
	var afterDecimalsDigits = 0;
	var compareCredeitAndDebitAccountsFlag = compareCredeitAndDebitAccountsOfficeId($("#fromAccountId").val(),$("#toAccountId").val());
	...
	if($("#fromOfficeHierarchyId").val() == ""){
		...
		$("#errorMsg").text("Please select Debit Account Head.");
		$(window).scrollTop(0);
		...
		$("#errorMsg").text("Please select Credit Head Head.");
		$(window).scrollTop(0);
	}else if(compareCredeitAndDebitAccountsFlag){
		$("#errorMsg").text("Please select Different Account Head.");
		$(window).scrollTop(0);
        ...
        document.getElementById("accountsMenuFormID").action=localStorage.contextPath+"/accounts/submitJournalTransaction";
	...
	ajaxVariable = $.ajax({
		...
		url: 'http://'+ajaxcallip+'/mfi/api/1.0/accounts/submitJournalTransaction',
		success: function(data) {
			if(data.journal == "success"){
				journalTransactionFormSubmission();
			}else if(data.journal == "failure"){
